...
export default function Navigation() {
    ...
    return (
        <>
            {/* Top Bar (Header) */}
            <header className="fixed top-0 left-0 right-0 md:left-72 z-40 h-16 md:h-20 glass border-b border-slate-200 flex items-center justify-between px-4 md:px-6 transition-all duration-300">
                <button
                    onClick={toggleMenu}
                    className="p-2.5 bg-violet-600 rounded-xl text-white hover:bg-violet-700 transition-all duration-300 shadow-lg shadow-violet-500/20 md:hidden"
            ...
            {/* Sidebar Menu */}
            <aside
                className={`fixed top-0 left-0 bottom-0 w-full md:w-72 glass-dark border-r border-slate-200 z-[55] flex flex-col pt-6 pb-8 shadow-2xl transition-transform duration-300 ease-in-out
                    ${isOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}
            >
                {/* Close Button Inside Sidebar - Mobile Only */}
                <div className="flex justify-start px-6 mb-4 md:hidden">
                    <button
                        onClick={toggleMenu}
