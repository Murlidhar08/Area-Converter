...
export default function Navigation() {
    ...
    return (
        <>
            {/* Top Bar (Header) */}
            <header className="fixed top-0 left-0 right-0 z-40 h-20 glass border-b border-slate-200 dark:border-white/5 flex items-center justify-between px-6">
                <button
                    onClick={toggleMenu}
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        exit={{ opacity: 0 }}
                        onClick={toggleMenu}
                        className="fixed inset-0 bg-slate-900/20 dark:bg-slate-950/60 backdrop-blur-sm z-[50]"
                    />
                )}
            </AnimatePresence>
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.aside
                        ...
                        className="fixed top-0 left-0 bottom-0 w-[280px] sm:w-[380px] glass-dark border-r border-slate-200 dark:border-white/5 z-[55] flex flex-col pt-6 pb-8 shadow-2xl"
                    >
                        {/* Close Button Inside Sidebar */}
                        <div className="flex justify-start px-6 mb-4">
                            <button
                                onClick={toggleMenu}
                                className="p-2 bg-slate-100 dark:bg-white/10 rounded-xl text-slate-500 dark:text-slate-400 hover:text-rose-600 dark:text-white transition-all transition-colors"
                            >
                                <X size={24} />
                            </button>
                        </div>
                                    ...
                                    <motion.button
                                        ...
                                        className={`w-full flex items-center gap-4 p-4 rounded-2xl transition-all duration-300 group
                                            ${isActive
                                                ...
                                                : "text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-white/5 hover:text-violet-600 dark:hover:text-white"}`}
                                    >
                                        <div className={`p-2 rounded-xl transition-colors ${isActive ? "bg-white/20" : "bg-slate-100 dark:bg-slate-800/50 group-hover:bg-slate-200 dark:group-hover:bg-slate-700"}`}>
                                            <Icon size={18} />
                                        </div>

                                        <div className="flex flex-col items-start flex-1">
                                            <span className="text-sm font-bold tracking-tight">{item.label}</span>
                                            <span className={`text-[10px] text-left ${isActive ? "text-violet-100" : "text-slate-500 dark:text-slate-400/60"}`}>
                                                {item.description}
                                            </span>
                                        </div>
                        ...
                        {/* Sidebar Footer */}
                        <div className="px-8 mt-auto pt-8 border-t border-slate-100 dark:border-white/5">
                            <a
                                href="https://github.com"
                                target="_blank"
                                rel="noreferrer"
                                className="flex items-center gap-3 text-slate-500 dark:text-slate-400 hover:text-violet-600 dark:hover:text-white transition-colors text-xs font-medium"
                            >
